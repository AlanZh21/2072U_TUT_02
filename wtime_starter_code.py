...
istart = 2
iend = 9
wtimerec= np.zeros((iend - istart + 1))                                 # Initialize an array for storing the wall times.       
for n in range(istart,iend+1):
    ...
    elapsed = time.time_ns() - start                        # Measure elapsed time.
...
for n in range(istart,iend+1):
    ...
    if ok==1:
        ...
        elapsed = start -  time.time_ns()                             # Measure elapsed time.
        print("n=%d wall time=%f det=%e" %(n,elapsed,detA))         # Output value of determinant.
